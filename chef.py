...
LICENSE_MAPPING = {
    ...
    "Non-commercial/non-Creative Commons (College Board)": licenses.SpecialPermissionsLicense(
        copyright_holder="Khan Academy",
...
class KhanAcademySushiChef(SushiChef):
    ...
    def construct_channel(self, **kwargs):
        ...
        root_topic = convert_ka_node_to_ricecooker_node(
            ka_root_topic, target_lang=lang_code
        )
    ...
    for playlist in youtube_channel["entries"]:
        if playlist:
            topic_node = nodes.TopicNode(
                source_id=playlist["id"], title=playlist["title"], description=""
            )
        ...
        for lang_code in subtitle_languages:
            if is_youtube_subtitle_file_supported_language(lang_code):
                if target_lang == "en":
                    files.append(
                        YouTubeSubtitleFile(
                            ka_node.translated_youtube_id, language=lang_code
                        )
                    )
